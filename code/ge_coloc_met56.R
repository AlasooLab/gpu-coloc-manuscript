...
all_signals = readr::read_tsv("data/ge_met56_results.tsv")

#Microrarray signals
microarray = dplyr::filter(all_signals, signal2 %like% "ILMN")
other_signals = dplyr::filter(all_signals, !(signal2 %like% "ILMN"))

#Separate out dataset ids and molecular trait ids
sep_signals = tidyr::separate(other_signals, signal2, c("dataset_id", "molecular_trait_id", "cs_index"), "_")
...
write.table(joined_data, "data/sup_tables/ge_vs_met56_coloc_results.tsv", sep = "\t", row.names = F, quote = F)

joined_data = readr::read_tsv("data/sup_tables/ge_vs_met56_coloc_results.tsv")
