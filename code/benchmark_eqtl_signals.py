...
metadata_path = '../data/tabix_ftp_paths.tsv'  # Adjust this if the file path changes
metadata = pd.read_csv(metadata_path, sep='\t')  # Assuming TSV format; adjust if different

metadata = metadata[metadata["study_label"]=="GTEx"]
...
for _, row in tqdm(metadata.iterrows(), desc="processing QTD"):
    ...
    QTS = row["study_id"]
    QTD = row["dataset_id"]
    QTS_PATH = "ADD PATH TO QTS FILE"
    # QTS_PATH = f"/gpfs/space/projects/eQTLCatalogue/qtlmap/{QTS_directory}/susie/{QTS}/{QTD}/{QTD}.lbf_variable.txt.gz"

    app_directory = "../data/benchmark_eqtl_signals"
    summary_file_path = "../data/benchmark_eqtl_metadata.tsv"

    process_gwas_file(QTS_PATH, QTD, app_directory, summary_file_path)