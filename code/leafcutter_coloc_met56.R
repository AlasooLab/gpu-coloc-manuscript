...
all_signals = readr::read_tsv("data/lc_met56_results.tsv")
...
split_signal = tidyr::separate(all_signals, signal2, c("dataset_id", "rest"), sep = "_", extra = "merge") %>%
...
write.table(joined_data, "data/sup_tables/leafcutter_vs_met56_coloc_results.tsv", sep = "\t", row.names = F, quote = F)
...
dplyr::filter(joined_data, gene_name == "BCAT2") %>% View()
